...
static void ggml_compute_forward_mul_mat(
    ...
    // 骁龙865专用优化路径 - 高层次cache和数据布局优化
    if (ggml_sd865_is_supported()) {
        // 调试：打印矩阵类型信息
        fprintf(stderr, "SD865: src0->type=%d (Q8_0=%d), src1->type=%d, threads=%d\n",
                src0->type, GGML_TYPE_Q8_0, src1->type, params->nth);

        if (src0->type == GGML_TYPE_Q8_0 && src1->type == GGML_TYPE_Q8_0) {
            fprintf(stderr, "SD865: Using cache-optimized GEMM for Q8_0×Q8_0 with %d threads\n", params->nth);
            ggml_compute_forward_mul_mat_sd865(params, dst);
            return;
        }
    }
